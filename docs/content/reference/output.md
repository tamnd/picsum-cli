...
describes what users get. Pick a format with `-o`, or let picsum choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
picsum <command> -o table   # aligned columns for reading
picsum <command> -o jsonl   # one JSON object per line, for piping
picsum <command> -o json    # a single JSON array
picsum <command> -o csv     # spreadsheet friendly
picsum <command> -o tsv     # tab-separated
picsum <command> -o url     # just the URL column
picsum <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
picsum <command> --fields id,title,url
```
...
```bash
picsum <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
picsum <command>            # a table, because this is a terminal
picsum <command> | wc -l    # JSONL, because this is a pipe
```
